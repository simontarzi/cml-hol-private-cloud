...
try:
    ...
    storage = "/HU/DEMO/" + os.getenv("HADOOP_USER_NAME") 
...
csv_file_path =  "/HU/DEMO/WA_Fn-UseC_-Telco-Customer-Churn-.csv"
...
if dataset_check.returncode != 0:
    # If the CSV file does not exist, create the necessary directory and copy the file
    run_cmd(f'hdfs dfs -mkdir -p {os.path.dirname(csv_file_path)}')
    run_cmd(f'hdfs dfs -copyFromLocal /home/cdsw/raw/WA_Fn-UseC_-Telco-Customer-Churn-.csv {csv_file_path}')

    # Set the STORAGE_MODE environment variable to "external"
...
os.environ["HIVE_DATABASE"] = "hol_demo"
hive_database = os.environ["HIVE_DATABASE"]
#hive_table = os.environ["HIVE_TABLE"]+"_"+USERNAME
hive_table = "CustomerChurn_"+USERNAME
...
yaml_text = f"""Churn Model API Endpoint:
        hive_table_qualified_names:                                             # this is a predefined key to link to training data
            - "{hive_table_fq}@cm"     # the qualifiedName of the hive_table object representing                
